...
- Compose 對下列目錄使用 host bind mount，讓資料位置可直接檢視、備份與搬移：
...
- Live collector 啟動前必須 fail closed 驗證：`LOB_DATA_ROOT` 是 absolute path、不是 `/`、存在專案建立的 `.lob-storage-root` marker、可由 service UID/GID 讀寫，而且 `findmnt`/`mountpoint` 證明它是預期的獨立 mount。若磁碟未掛載，不得退回系統碟上的同名普通目錄。
...
停止 collector 後，`private-runtime` 整個目錄也必須可以由管理者直接刪除並在下次啟動時以正確權限重建；管理 script 的價值是先做盤點、保護行情資料並降低誤刪風險，不把它變成唯一清理方式。
