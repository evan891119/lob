...
| Phase 4 | 工具完成；pilot 待執行 | `pilot-report` 查詢 ClickHouse rows、平均/尖峰 EPS、latency、parts、session/gap，分開輸出 `bytes_on_disk`、壓縮前後 data bytes、`uncompressed/compressed` ratio 與最低／建議 dataset scope checks，並用實際可用容量及觀測 `bytes_on_disk`/day 估算 80%/90% 水位與 retention days | 3–5 商品至少一完整交易日、建議五日，確認完整時段並據實決定 retention/backup |
...
- Pilot tool fixture proof：LOB/Tick 各 `181` rows、fixture latency p50/p95/p99 分別為 `5/5/5 ms` 與 `7/7/7 ms`；工具另輸出每商品/日期 peak EPS、parts `bytes_on_disk`、壓縮前後 data bytes、`uncompressed/compressed` ratio、觀測商品／交易日、最低 3 商品＋1 日與建議 5 日 dataset scope，以及按平均磁碟占用計算的 90% 水位 retention days。20TB decimal usable bytes 對應 80% `16,000,000,000,000`、90% `18,000,000,000,000`。這些 fixture 數字不代表真實市場容量或完整交易時段。
