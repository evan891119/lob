...
| --- | --- | --- | --- |
| Phase 0 | 本機完成；目標 Linux 當下 storage/credential metadata gate 通過 | Git `main`、fully pinned Python/Docker、Compose config、non-root/read-only fixture container、root RO + nested RW mounts、Linux tmpfs UID/mode/marker/live fail-closed、containerized privacy controls、tracked-file scan、read-only acceptance check；目標 Linux 舊版 acceptance wrapper 已通過 mount/layout/marker、Compose config 與 repo 外 credential absolute/regular/non-symlink/`0600` metadata 驗證；新版 wrapper 另 fail-closed 要求 owner `10001:10001`，並提供不洩漏 device/UUID 的 filesystem size/UUID/`fstab` check 與暫存 boot ID 只落 private-runtime 的 reboot prepare/verify check | 在目標 filesystem 重跑新版 exact credential owner/mode 與 identity check，並執行 reboot prepare/verify |
