...
| Phase 5 | 本機完成 | BidAsk+Tick 以 security type＋exchange＋symbol＋時間區間查詢；全商品日匯出按完整 market identity 建立 Zstd Parquet Hive partitions，同代碼跨市場不共用檔案；DuckDB `union_by_name`、duplicate/order/time gap/timestamp/crossed book/negative volume checks；sequence gap 只在 explicit complete scope 計算，partial scope 回傳 `null` | 用實際 pilot dataset 的全商品日匯出重跑並保存結果 |

### 本機驗證摘要

- Host unit tests：115 tests passed；不使用 Shioaji credential，包含 Shioaji 1.5 legacy contract lookup、FUT/OPT target metadata、FOP optional fields、safe login/lookup/resubscribe category、全訂閱失敗診斷、concurrent health atomic replace、逐商品／stream 首末活動時間與 session coverage、collector process CPU/RSS 與 allowlisted Docker resource metrics、ClickHouse mixed-stream partial write 與 uncertain-commit idempotent replay、pilot typed date isolation/market-identity/compression/retention/scope/projection/成功訂閱商品存在性/每商品每日雙 stream 與 wrapper fail-closed 邊界、quality partial/full sequence scope 與跨 stream duplicate identity、Parquet complete-identity query/partition/path validation、unsafe identity pre-connect rejection、acceptance report no-leak/completed-session reconciliation、credential exact owner/mode fail-closed、direct/shared-bind storage identity no-leak、Docker boot readiness enabled/active/running/restart-policy fail-closed/no-leak、database/network outage drills 與 reboot persistence fail-closed/no-leak checks。
