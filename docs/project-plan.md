...
| Phase 3 | 本機完成；STK＋FUT 兩商品 live gate 完成；長時間 gate | 多商品 loader、FUT/OPT callbacks、`TXFR1` logical/resolved/target metadata mapping、FOP optional diff handling、connection gap/reconnect counter、exponential login backoff、durable market/audit spool、同 process 自動 replay、short Docker outage 與 privacy purge 實測；目標 Linux 同時啟用 `2330`/`TXFR1`，四個 BidAsk/Tick subscriptions 全成功，實際 FUT `TXFH6` 與 STK 兩種 stream 均有 rows；另提供明確確認、30–900 秒安全界線、exit recovery trap 與 privacy-safe delta report 的單指令 ClickHouse outage drill | 目標 Linux OPT、網路斷線、ClickHouse/host restart、執行長時間 outage/replay drill |
...
- Host unit tests：71 tests passed；不使用 Shioaji credential，包含 Shioaji 1.5 legacy contract lookup、FUT/OPT target metadata、FOP optional fields、safe login/lookup category、全訂閱失敗診斷、pilot retention 邊界、acceptance report no-leak/completed-session reconciliation、credential metadata fail-closed、storage identity no-leak 與 outage drill fail-closed/no-leak checks。
