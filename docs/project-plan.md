...
| Phase 4 | 工具完成；pilot 待執行 | collector session/health 保存 session-relative CPU seconds 與 process max RSS；`pilot-report` 查詢 rows、平均/尖峰 EPS、latency、collector average CPU/max RSS、parts、session/gap，按商品類型／市場／代碼區分商品，輸出 `bytes_on_disk`、壓縮前後 data bytes、`uncompressed/compressed` ratio 與最低／建議 dataset scope checks；另以明列假設推估 10／50／100 商品的每日／月／年容量、aggregate EPS、單份 full-copy backup bytes 及實際可用容量 90% 水位 retention days | 3–5 商品至少一完整交易日、建議五日，確認完整時段、另量測 ClickHouse container CPU/memory，據實決定 retention/backup/整機運算資源 |
...
- Pilot tool fixture proof：LOB/Tick 各 `181` rows、fixture latency p50/p95/p99 分別為 `5/5/5 ms` 與 `7/7/7 ms`；collector process CPU seconds/max RSS 可持久化，report 依 session duration 產生 average CPU percent。工具另輸出每商品/日期 peak EPS、parts `bytes_on_disk`、壓縮前後 data bytes、`uncompressed/compressed` ratio、按商品類型／市場／代碼計算的觀測商品數、最低 3 商品＋1 日與建議 5 日 dataset scope，以及 10／50／100 商品的 20／250 交易日容量、aggregate EPS、單份 full-copy backup 與 90% retention projections。20TB decimal usable bytes 對應 80% `16,000,000,000,000`、90% `18,000,000,000,000`。所有線性／peak-sum／backup 假設都寫入 report；fixture 不代表真實市場容量、完整交易時段或 ClickHouse container 資源。
