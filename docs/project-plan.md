...
| Phase 5 | 本機完成 | BidAsk+Tick 以 security type＋exchange＋symbol＋時間區間查詢；全商品日匯出按完整 market identity 建立 Zstd Parquet Hive partitions，同代碼跨市場不共用檔案；DuckDB `union_by_name`；quality 的排序／時間 gap 按 session＋完整 market identity＋stream 隔離，duplicate/sequence 仍按 session-global identity 判斷，另檢查 invalid identity/stream、timestamp、crossed book、negative volume 與每個 identity 雙 stream coverage；sequence gap 只在 explicit complete scope 計算，partial scope 回傳 `null`，完整 scope 可原子保存 report 並以非零狀態 fail closed | 用實際 pilot dataset 的全商品日匯出重跑並保存 complete-scope quality report |

### 本機驗證摘要

- Host unit tests：120 tests passed；不使用 Shioaji credential，包含 Shioaji 1.5 legacy contract lookup、FUT/OPT target metadata、FOP optional fields、safe login/lookup/resubscribe category、全訂閱失敗診斷、concurrent health atomic replace、逐商品／stream 首末活動時間與 session coverage、collector process CPU/RSS 與 allowlisted Docker resource metrics、ClickHouse mixed-stream partial write 與 uncertain-commit idempotent replay、pilot typed date isolation/market-identity/compression/retention/scope/projection/成功訂閱商品存在性/每商品每日雙 stream 與 wrapper fail-closed 邊界、quality partial/full sequence scope、跨 stream duplicate identity、跨市場同代碼 isolation、identity/stream/timezone/雙 stream completeness 與 CLI report fail-closed、Parquet complete-identity query/partition/path validation、unsafe identity pre-connect rejection、acceptance report no-leak/completed-session reconciliation、credential exact owner/mode fail-closed、direct/shared-bind storage identity no-leak、Docker boot readiness enabled/active/running/restart-policy fail-closed/no-leak、database/network outage drills 與 reboot persistence fail-closed/no-leak checks。
