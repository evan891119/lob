...
| Phase 2 | 本機完成；collector rebuild 持久性與目標 filesystem 容量實測通過；完整交易時段 gate | bounded queue/batch metrics、bytes/inode capacity gauges、ClickHouse fixture insert、session/gap latest views、idempotent migrator、graceful stop、93.82% Linux tmpfs capacity stop、bind-mount persistence；market tables 部分成功時只 spool 未確認 stream，replay 以 session＋sequence identity 跳過 uncertain commit 與 interrupted replay 已存在事件；目標 Linux 新 session 啟動後仍查到更早的歷史 rows，當下 storage used `8.12%`、drop `0`、open gap `0`；shared ext4 filesystem 實測 total `19,919,910,756,352` bytes、service usable `18,919,865,425,920` bytes；acceptance report 可在不輸出 session UUID 下直接核對最近 completed session 的 DB rows/received/drop/open-gap；reboot verifier 可核對 boot/session 改變、歷史起點與 rows/health/subscription persistence | 部署 replay 修正後，以新的完整交易時段 session 重跑 completed-session reconciliation；host reboot persistence 由部署者因共用主機決定延後 |
| Phase 3 | 本機完成；STK＋FUT 兩商品 live gate 完成；長時間 gate | 多商品 loader、FUT/OPT callbacks、`TXFR1` logical/resolved/target metadata mapping、FOP optional diff handling、connection gap/reconnect counter、exponential login backoff、event `13` 後 main-thread resubscribe/retry、durable market/audit spool、同 process 自動 replay、short Docker outage 與 privacy purge 實測；目標 Linux 同時啟用 `2330`/`TXFR1`，四個 BidAsk/Tick subscriptions 全成功，實際 FUT `TXFH6` 與 STK 兩種 stream 均有 rows；另提供明確確認、時間安全界線、exit recovery trap 與 privacy-safe delta report 的單指令 ClickHouse/outbound-network drills | 目標 Linux OPT、執行 outbound network drill、ClickHouse/host restart、執行長時間 outage/replay drill |
...
- Host unit tests：105 tests passed；不使用 Shioaji credential，包含 Shioaji 1.5 legacy contract lookup、FUT/OPT target metadata、FOP optional fields、safe login/lookup/resubscribe category、全訂閱失敗診斷、concurrent health atomic replace、process CPU/RSS metrics、ClickHouse mixed-stream partial write 與 uncertain-commit idempotent replay、pilot typed date isolation/market-identity/compression/retention/scope/projection 邊界、quality partial/full sequence scope 與跨 stream duplicate identity、Parquet complete-identity query/partition/path validation、unsafe identity pre-connect rejection、acceptance report no-leak/completed-session reconciliation、credential exact owner/mode fail-closed、direct/shared-bind storage identity no-leak、Docker boot readiness enabled/active/running/restart-policy fail-closed/no-leak、database/network outage drills 與 reboot persistence fail-closed/no-leak checks。
...
Source、fixture、Docker/Compose 與本機可重現驗證已完成；目標 Linux 當下 mount/layout/credential metadata、shared ext4 bind chain 與實際容量、Docker boot readiness、STK＋FUT 兩商品 live streams，以及 collector rebuild 後歷史資料持久性，亦已由部署者提供的 privacy-safe report 證實。部署者基於共用主機決定不為驗收刻意 reboot；此項維持未實測、不冒充通過，但不阻止後續 Phase 的外部驗收。整體 goal 尚未完成：仍需 OPT、完整交易時段 reconciliation、ClickHouse restart 與長時間 outage/replay 測試，以及 3–5 商品 pilot 與實際 retention/backup 決策；若未來自然 reboot，再補做 persistence 證據。在這些條件完成前，不把 Phase 2、3、4 的外部驗收標為完成，也不要求使用者把 credential 提供給 Codex。
