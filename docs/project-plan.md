...
- Pilot tool fixture proof：LOB/Tick 各 `181` rows、fixture latency p50/p95/p99 分別為 `5/5/5 ms` 與 `7/7/7 ms`；工具另輸出每商品/日期 peak EPS、parts `bytes_on_disk`、壓縮前後 data bytes、`uncompressed/compressed` ratio、按商品類型／市場／代碼計算的觀測商品數、最低 3 商品＋1 日與建議 5 日 dataset scope，以及 10／50／100 商品的 20／250 交易日容量、aggregate EPS、單份 full-copy backup 與 90% retention projections。20TB decimal usable bytes 對應 80% `16,000,000,000,000`、90% `18,000,000,000,000`。所有線性／peak-sum／backup 假設都寫入 report；這些 fixture 數字不代表真實市場容量或完整交易時段。
