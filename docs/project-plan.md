...
| Phase 5 | 本機完成 | BidAsk+Tick 時間區間 event query、全商品日匯出、Zstd Parquet、DuckDB `union_by_name`、duplicate/order/time gap/timestamp/crossed book/negative volume checks；sequence gap 只在 explicit complete scope 計算，partial scope 回傳 `null`，避免多商品合法交錯被誤報 | 用實際 pilot dataset 的全商品日匯出重跑並保存結果 |

### 本機驗證摘要

- Host unit tests：96 tests passed；不使用 Shioaji credential，包含 Shioaji 1.5 legacy contract lookup、FUT/OPT target metadata、FOP optional fields、safe login/lookup/resubscribe category、全訂閱失敗診斷、concurrent health atomic replace、process CPU/RSS metrics、pilot typed date isolation/market-identity/compression/retention/scope/projection 邊界、quality partial/full sequence scope 與跨 stream duplicate identity、acceptance report no-leak/completed-session reconciliation、credential exact owner/mode fail-closed、direct/shared-bind storage identity no-leak、Docker boot readiness enabled/active/running/restart-policy fail-closed/no-leak、database/network outage drills 與 reboot persistence fail-closed/no-leak checks。
...
- Research path：全商品日匯出兩張 Zstd Parquet 共 `362` 列；DuckDB 可查；在 explicit complete sequence scope 下 quality 七類 counters 全為 `0`。單商品／部分 stream 匯出不再臆測 sequence 完整性，會以 `sequence_gaps=null` 明確標示 unavailable。
