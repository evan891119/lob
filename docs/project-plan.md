...
| Phase 2 | 本機完成；collector rebuild 持久性與目標 filesystem 容量實測通過；完整交易時段 gate | bounded queue/batch metrics、bytes/inode capacity gauges、ClickHouse fixture insert、session/gap latest views、idempotent migrator、graceful stop、93.82% Linux tmpfs capacity stop、bind-mount persistence；目標 Linux 新 session 啟動後仍查到更早的歷史 rows，當下 storage used `8.12%`、drop `0`、open gap `0`；shared ext4 filesystem 實測 total `19,919,910,756,352` bytes、service usable `18,919,865,425,920` bytes；acceptance report 可在不輸出 session UUID 下直接核對最近 completed session 的 DB rows/received/drop/open-gap；reboot verifier 可核對 boot/session 改變、歷史起點與 rows/health/subscription persistence | 一個完整交易時段結束後執行 completed-session reconciliation；host reboot persistence 由部署者因共用主機決定延後 |
...
- Host unit tests：101 tests passed；不使用 Shioaji credential，包含 Shioaji 1.5 legacy contract lookup、FUT/OPT target metadata、FOP optional fields、safe login/lookup/resubscribe category、全訂閱失敗診斷、concurrent health atomic replace、process CPU/RSS metrics、pilot typed date isolation/market-identity/compression/retention/scope/projection 邊界、quality partial/full sequence scope 與跨 stream duplicate identity、Parquet complete-identity query/partition/path validation、unsafe identity pre-connect rejection、acceptance report no-leak/completed-session reconciliation、credential exact owner/mode fail-closed、direct/shared-bind storage identity no-leak、Docker boot readiness enabled/active/running/restart-policy fail-closed/no-leak、database/network outage drills 與 reboot persistence fail-closed/no-leak checks。
