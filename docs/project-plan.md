...
- 正式 Linux 主機建議使用 ext4 或 XFS，並以 UUID 寫入 `/etc/fstab`，讓 20TB filesystem 在重新開機後維持相同 mount point。若 filesystem 共用，`LOB_DATA_ROOT` 使用專案子目錄的 exact bind mount，且 `/etc/fstab` 必須同時保存底層 UUID mount 與 `x-systemd.requires-mounts-for=<底層 mount point>` 的 bind dependency。實際掛載與 `/etc/fstab` 修改由主機管理者完成，不由 collector 自動執行。
- Live collector 啟動前必須 fail closed 驗證：`LOB_DATA_ROOT` 是 absolute path、不是 `/`、存在內容完全相符的 `.lob-storage-root` marker、root 可由 service UID/GID 讀取、三個 collector 子目錄可寫，而且 `findmnt`/`mountpoint` 證明它是 exact direct mount，或是來源、UUID 與 systemd dependency 都相符的 shared-filesystem bind mount。若底層磁碟未掛載，不得退回系統碟上的同名普通目錄。
...
- ClickHouse 實際 `bytes_on_disk`、壓縮前後 data bytes 與壓縮率。
- queue 尖峰使用率。
...
- 建立 privacy-safe Docker boot readiness checker，唯讀驗證 `docker.service` enabled/active、Compose config、必要 services 運行狀態與實際 container restart policies；不得以靜態 readiness 冒充 reboot persistence。
...
- Parquet 以 `security_type/exchange/symbol/trading_date` 完整公開 market identity 分區；同代碼跨市場不得落入同一輸出檔。
...
| --- | --- | --- | --- |
| Phase 0 | 本機完成；目標 Linux 當下 storage/credential metadata、shared-bind identity 與 Docker boot readiness gates 通過 | Git `main`、fully pinned Python/Docker、Compose config、non-root/read-only fixture container、root RO + nested RW mounts、Linux tmpfs UID/mode/marker/live fail-closed、containerized privacy controls、tracked-file scan、read-only acceptance check；目標 Linux acceptance wrapper 已通過 mount/layout/marker、Compose config 與 repo 外 credential absolute/regular/non-symlink/owner `10001:10001`/mode `0600` metadata 驗證；shared ext4 filesystem 的 exact bind source、底層 UUID mount 與 explicit systemd dependency 亦由 privacy-safe identity report 證實；Docker service enabled/active、必要 services running 與實際 restart policies 由 privacy-safe boot readiness report 證實；reboot prepare/verify 的暫存 boot ID 只落 private-runtime | 實際 reboot persistence 由部署者因共用主機決定延後，不標示為已驗證 |
...
| Phase 2 | 本機完成；collector rebuild 持久性、目標 filesystem 容量與自然 replay reconciliation 實測通過；完整交易時段邊界仍由部署者確認 | bounded queue/batch metrics、bytes/inode capacity gauges、ClickHouse fixture insert、session/gap latest views、idempotent migrator、graceful stop、93.82% Linux tmpfs capacity stop、bind-mount persistence；market tables 部分成功時只 spool 未確認 stream，replay 以 session＋sequence identity 跳過 uncertain commit 與 interrupted replay 已存在事件；目標 Linux 新 session 啟動後仍查到更早的歷史 rows；shared ext4 filesystem 實測 total `19,919,910,756,352` bytes、service usable `18,919,865,425,920` bytes；修正版 completed session 的 received/market rows `1,089,905/1,089,905`、spooled/replayed `28/28`、drop/open gap `0`，自然實機 reconciliation 通過；acceptance 另輸出每個成功訂閱 stream 的 session-scoped rows/首末時間 | 以各市場實際時段確認 completed session coverage；host reboot persistence 由部署者因共用主機決定延後 |
| Phase 3 | 本機完成；STK＋三種 FUT 多商品 live gate 完成；OPT 與受控故障 gate | 多商品 loader、FUT/OPT callbacks、continuous futures logical/resolved/target metadata mapping、FOP optional diff handling、connection gap/reconnect counter、exponential login backoff、event `13` 後 main-thread resubscribe/retry、durable market/audit spool、同 process 自動 replay、short Docker outage 與 privacy purge 實測；目標 Linux 同時啟用 `2330`、`TXFR1`、`MXFR1`、`TMFR1`，八個 BidAsk/Tick subscriptions 全成功且四個實際商品有 rows；health/acceptance 現在逐商品／stream 記錄首末活動時間，避免把 startup subscription count 當成持續 liveness | 目標 Linux OPT、執行 outbound network drill、ClickHouse/host restart、執行長時間 outage/replay drill |
| Phase 4 | 工具完成；pilot 待 scoped 實機驗收 | collector session/health 保存 session-relative CPU seconds 與 process max RSS；`pilot-report` 以 typed inclusive date scope 隔離 market/peak/session/gap，按商品類型／市場／代碼區分商品，查詢 rows、EPS、latency、collector average CPU/max RSS 與 parts；無日期時輸出 exact active-part totals，日期範圍則明確標示按 table row share 估算 scoped `bytes_on_disk`／壓縮 bytes，並保留 global exact totals；另輸出最低／建議 scope、每商品每日期雙 stream、simulation/completed/no-drop/no-open-gap/storage projection checks，移除 session UUID，且不把 rows 冒充完整交易時段；單一 `scripts/pilot-check` 先重跑 acceptance、產生 scoped report，再以三次 allowlisted Docker stats 產生不含 container/host identity 的 collector/ClickHouse point-in-time CPU/memory report，raw samples 只暫存 private-runtime 並由 trap 清除；容量 projections 明列 10／50／100 商品每日／月／年、aggregate EPS、單份 full-copy backup bytes 與 90% retention days | 選定一段沒有股票靜默缺流的 3–5 商品完整交易日（建議五日）執行 pilot-check，確認完整時段；在代表性高流量時段重跑資源樣本或搭配外部長期監控，據實決定 retention/backup/整機運算資源 |
| Phase 5 | 本機完成 | BidAsk+Tick 以 security type＋exchange＋symbol＋時間區間查詢；全商品日匯出按完整 market identity 建立 Zstd Parquet Hive partitions，同代碼跨市場不共用檔案；DuckDB 範例以完整 identity glob 同時讀 LOB/Tick、filename 還原 stream、Hive partitioning＋`union_by_name` 合併 schema，並限制 identity/date/半開時間區間；quality 的排序／時間 gap 按 session＋完整 market identity＋stream 隔離，duplicate/sequence 仍按 session-global identity 判斷，另檢查 invalid identity/stream、timestamp、crossed book、negative volume 與每個 identity 雙 stream coverage；sequence gap 只在 explicit complete scope 計算，partial scope 回傳 `null`，完整 scope 可原子保存 report 並以非零狀態 fail closed | 用實際 pilot dataset 的全商品日匯出重跑並保存 complete-scope quality report；在 pinned container/目標 Linux 實際執行新版 DuckDB 雙 stream SQL |

### 本機驗證摘要

- Host unit tests：122 tests passed；不使用 Shioaji credential，包含 Shioaji 1.5 legacy contract lookup、FUT/OPT target metadata、FOP optional fields、safe login/lookup/resubscribe category、全訂閱失敗診斷、concurrent health atomic replace、逐商品／stream 首末活動時間與 session coverage、collector process CPU/RSS 與 allowlisted Docker resource metrics、ClickHouse mixed-stream partial write 與 uncertain-commit idempotent replay、pilot typed date isolation/market-identity/compression/retention/scope/projection/成功訂閱商品存在性/每商品每日雙 stream 與 wrapper fail-closed 邊界、quality partial/full sequence scope、跨 stream duplicate identity、跨市場同代碼 isolation、identity/stream/timezone/雙 stream completeness 與 CLI report fail-closed、Parquet complete-identity query/partition/path validation、DuckDB 雙 stream glob/filename/identity/date/time static contract、unsafe identity pre-connect rejection、acceptance report no-leak/completed-session reconciliation、credential exact owner/mode fail-closed、direct/shared-bind storage identity no-leak、Docker boot readiness enabled/active/running/restart-policy fail-closed/no-leak、database/network outage drills 與 reboot persistence fail-closed/no-leak checks。
...
- Target Linux privacy-safe acceptance report（部署者提供，`2026-07-20 13:19 +08:00`）：health fresh/running、`simulation=true`、`2330` 與 `TXFR1→TXFH6` 共四個 subscriptions active、failed `0`；STK/FUT 各自 BidAsk/Tick rows 皆存在，總計 LOB `79,657`、Tick `18,576`，兩個實際 symbols、一個 trading day。當下 session received/written 為 `2,283/2,282`、queue `0/20,000`、high-water `15`、drop/spool/replay/reconnect/clock anomaly 均 `0`；三個既有 `capture_gap` intervals 全部 closed、affected count `0`，storage used `8.12%`。一筆 received/written 差值視為執行中 batch 邊界，不冒充完整時段 reconciliation。
- Target Linux completed-session reconciliation（部署者提供，`2026-07-22`）：`2330`、`TXFR1`、`MXFR1`、`TMFR1` 共八個 BidAsk/Tick subscriptions 成功，session graceful stop、drop/notice drop/open gap 均為 `0`；received `4,634,005`、written `4,633,644`、spooled/replayed `361/361`，但 market rows `4,634,032`，多出 `27` rows，因此 gate 正確 fail closed。數字與 mixed-stream sink 先成功寫入一張 table、後一張失敗後整批 replay 的路徑吻合；本機已新增 partial-write accounting 與 identity-aware replay，仍待部署後以新 completed session 重驗。舊 duplicate 不會由升級自動刪除。
- Target Linux 修正版自然 replay reconciliation（部署者提供，`2026-07-24 22:04 +08:00`）：最近 completed session received `1,089,905`、written `1,089,877`、spooled/replayed `28/28`、market rows `1,089,905`，drop/notice drop/open gap 均為 `0`，completed-session reconciliation 全部通過；這關閉上述 27-row replay defect 的新-session gate，舊 duplicate 仍不自動刪除。資料庫全域已觀測四個實際商品與五個 trading days，但完整交易時段仍須依各市場的 session 首末時間判定。
- Per-stream liveness evidence（部署者提供）：同份 report 中 `2330` 的末筆資料停在期貨仍持續更新之前；重啟 collector 後，部署者以連續查詢確認 `2330` rows 再次增加。這證明當下股票流已恢復，也揭露 `subscriptions_active=8` 只代表 startup 成功。新版 health/acceptance 因此加入每個實際商品／stream 的 received、首末時間、age 及 completed-session coverage；不同市場休市時間不同，不以單一固定 stale 秒數自動判定故障。
- Collector rebuild persistence（由同一 report 推論）：latest session 起於 `13:16`，但 database 最早 event 為 `10:53`，且 report 已含新版 per-security-type checks，因此 collector image/session 更新後仍可查到更新前 rows；這只證明 collector rebuild/recreate 未破壞 ClickHouse 歷史資料，不等同 host reboot 或 ClickHouse data-directory restore 驗收。
- Target Linux privacy-safe storage identity（部署者提供，`2026-07-20`）：`mount_layout=bind`、`filesystem_type=ext4`，底層 UUID、`/etc/fstab` UUID entry、bind source 與 explicit systemd dependency 全部驗證通過；filesystem total `19,919,910,756,352` bytes、service usable `18,919,865,425,920` bytes。Report 未包含 UUID 值、device、hostname 或 host path；此結果證明當下設定與實際容量，不等同 reboot persistence。
- Target Linux privacy-safe Docker boot readiness（部署者提供，`2026-07-20`）：`docker.service` enabled/active、Compose config valid、ClickHouse/collector running、實際 restart policies 全部為預期的 `unless-stopped`。Report 未包含 host path、container ID 或 hostname；此結果證明靜態開機恢復條件，不等同實際 reboot persistence。
...
- Research path：既有全商品日匯出兩張 Zstd Parquet 共 `362` 列且 DuckDB 可查；新版 exporter 另以 unit proof 確認完整 market identity query/partition，同代碼 STK/FUT 不共用檔案。在 explicit complete sequence scope 下 quality 七類 counters 全為 `0`；單商品／部分 stream 匯出以 `sequence_gaps=null` 明確標示 unavailable。新版 layout 尚待用實際 pilot dataset 重跑。
- Pilot tool fixture proof：LOB/Tick 各 `181` rows、fixture latency p50/p95/p99 分別為 `5/5/5 ms` 與 `7/7/7 ms`；collector process CPU seconds/max RSS 可持久化，report 依 session duration 產生 average CPU percent。Typed date scope 隔離行情與 audit，10% table row-share case 將 global `10,000` bytes 明確估為 scoped `1,000` bytes，無 filter 則標為 exact active parts。工具另輸出每商品/日期 peak EPS、壓縮率、最低／建議 dataset scope，以及 10／50／100 商品的 20／250 交易日容量、aggregate EPS、單份 full-copy backup 與 90% retention projections；獨立 resource report 只輸出 collector/ClickHouse point-in-time CPU/memory aggregates。20TB decimal usable bytes 對應 80% `16,000,000,000,000`、90% `18,000,000,000,000`。所有估算方法與樣本限制都寫入 report；fixture 不代表真實市場容量、完整交易時段或目標 Linux 資源。
...
- External acceptance path：單一 read-only script 先以 metadata-only 驗證 repo 外 credential path/type/owner `10001:10001`/mode `0600` 與 storage/Compose，再將 health、market rows、latest session 與 gap aggregates 寫入 Parquet 區的 allowlisted JSON；STK/FUT/OPT 各自有雙 stream check，並刻意忽略 credential 內容/path、session ID、未知欄位與非公開 token，且不把當下摘要冒充 reboot/outage/full-day 證據。
