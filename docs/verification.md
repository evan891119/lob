...
- Python 3.14 host 上以 `PYTHONPATH=src` 執行 122 個 stdlib unit tests。
...
- 查詢 SQL：同一 security type/exchange/symbol/date/time range 實際合併回傳 BidAsk 與 Tick event envelope，並以 event/received/session/sequence 穩定排序。Parquet exporter 以完整 market identity 查詢及建立 Hive partitions，同代碼跨 STK/FUT/OPT 或 exchange 不會共用輸出檔。DuckDB SQL 範例不再只讀 LOB；它以完整 identity glob 同時讀兩種檔案、從 filename 還原 stream、使用 Hive partitioning/union-by-name，並限制 identity/date/半開時間區間。Static contract tests 鎖定這些必要子句；本機沒有 DuckDB executable，因此新版 SQL 的實際執行仍待 pinned container 或目標 Linux 的 Phase 5 匯出驗收，不冒充 runtime proof。
...
- Partial-write replay proof：ClickHouse 兩張 market tables 的寫入會回報已確認成功的 stream，只 spool 尚未確認的尾端；replay 在每張目標 table 以 `session_id + sequence_no` 過濾既有事件。測試同時涵蓋 Tick insert 在 commit 前失敗，以及 server 已 commit、client 才收到連線錯誤兩種情況；重複 replay 不增加 duplicate identity。
...
- Container resource evidence proof：resource parser 只接受固定 `clickhouse`／`collector` service label、至少三組對齊的 sample number，以及 CPU/memory/limit/percent 數值；支援 Docker stats 的 decimal/binary size units，缺 service、樣本不足、memory limit 改變或帶入其他 label 都 fail closed 且錯誤不回顯輸入。`pilot-check` 的原始樣本使用 `0600` private-runtime temporary file 與 exit/interrupt trap 清除，report 不含 container ID/name、hostname 或 host path，並明列 point-in-time 限制。
