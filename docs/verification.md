...
- Python 3.14 host 上以 `PYTHONPATH=src` 執行 82 個 stdlib unit tests。
...
- Pilot report unit proof：合併 average/peak EPS，分開計算 ClickHouse parts 的 `bytes_on_disk`、壓縮前後 data bytes 與 `uncompressed/compressed` ratio；retention 只使用實際磁碟占用，另輸出至少 3 商品＋1 日與建議 5 日 dataset scope checks。空資料集不臆測壓縮率或保留天數。
